...
| **Pololu D24V22F12 — 12V dedicated L2 LiDAR rail** | **$19** | 🆕 Order. New in v3.4 (Option A split). 36V Vin max. ⚠️ **CURRENT RATING DISPUTED 2026-08-18:** this row said "2.6A max ... ~2.6× headroom", but Pololu's own spec page (item 2855) states **1.9 A at 24 V in**. The L2's ~1 A still fits either way, but the stated headroom does not follow from the vendor's number — reconcile before relying on it. Size is now Pololu-sourced too: **17.78 × 17.78 × 7.87 mm**, correcting the 6.0 height in dimensions.md (see #366). Clean power for LiDAR — no servo transient ringing. LC filter retained on the L2-buck output. |
...
| **leg_v6 servo screws — M2/M2.5, 6 lengths (MEASURED 2026-07-11, `docs/fastener-schedule.md`)** | **$18** | 🆕 Order — **12 active servos (4 HAA + 4 HFE + 4 KFE)** need 3 fastener families each (144 screws total, self-consistent at 12/servo): **case-mount M2 self-tap** ×40 M2×22 (HAA/KFE + HFE-near-pair) + ×8 M2×25 (HFE far pair only — femur's LA-6 underside ramp deepens that pair's floor +4.4mm, measured 6.525mm vs 2.125mm elsewhere; capped at 25 so it doesn't bottom the ~19.9mm case column); **horn M2.5** (driven side, into the ~3.05mm horn disc) ×32 M2.5×6 (HAA+KFE) + ×16 M2.5×5 (HFE — thinner LA-7 backing wall); **wheel M2.5** (idler side, into the ~2.1mm wheel disc, **no center screw** — idler boss only) ×32 M2.5×8 (HFE+KFE) + ×16 M2.5×14 (HAA — shoulder's long wheel-boss reach). M2.5×14 is an uncommon length, likely a separate order from the generic M2/M2.5 pack; add ~10% spares per length. **Confirmed too short: stock STS3215 case screws** (already known); **no verified stock length found for the horn/wheel M2.5** — source all 4 M2.5 lengths, don't assume the servo kit covers any of them (check the hardware baggie before ordering). Stock retention screw (Ø5.4×~1.5mm proud, ×1/horn = 12) ships with the servo/horn, not sourced. |
...
| **Mezzanine standoffs — 8× M3 × 20 mm M-F brass + M3 screws** | $12 | ⚠️ **CORRECTED 2026-08-18 — this said 4, and the design needs 8.** There are TWO 20 mm gaps, not one, on the same 74 × 66 pattern at (−40.5, ±33) and (33.5, ±33), stacked vertically: **4 LOWER** (floor plate z6.0 → power board z26.0, `STANDOFF_FLOOR_MM`) and **4 UPPER** (power board z27.6 → logic board z47.6, `STANDOFF_PB_LB_MM`). The old row ordered the upper set but justified it with the LOWER gap's clearances — the Ø10×17 bulk caps and the buck cards both live under the power board, not above it. The lower 20 mm is set by the caps (16 mm left them 1 mm proud); the cards are **9.02 mm** (Pololu, not the ~15 this row used to claim). ~~Total stack ≈ 41 mm < 46.9 mm trunk~~ **wrong comparison**: the real ceiling is `STACK_TOP_Z ≈ 62.22` (Teensy/Nano socket top, measured off the parsed board mesh) against a trunk plateau of 46.91 — the stack was never meant to fit inside the trunk, it runs up into the riser bay and clears the deck underside (67.9) by 5.68 mm. 📦 **RESOLVED 2026-08-18 — Aiden has a 40-piece M3 M-F set and they screw into each other.** So the requirement is not 8 single 20 mm pieces, it is **8 STACKS that each total 20 mm** (2×10, or 10+6+4, whatever the kit holds). No purchase needed. ⚠️ **But 20 mm is not a preference — each gap has a hard floor at ~18 mm, for two DIFFERENT reasons:** *lower gap* — the Ø10×17 bulk caps hang off the board's underside, so below ~18 mm they hit the floor plate (`floor_plate.scad` records the window as S ∈ [~18, 24.7]); *upper gap* — `Q1` is an 18 mm TO-220 standing on the board top, so below ~18 mm it hits the logic board underside. At the designed 20 mm the gate measures **3.00 mm** clearance under the caps and **2.00 mm** over Q1 (and only **1.40 mm** from Q1 to the logic board's own B.Cu parts) — so there is little to give away. Build each stack to 20, and put **Loctite 243** on the intermediate joints: a chain of threaded joints in a walking robot has more places to back off than a single post does. |
