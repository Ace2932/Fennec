...
#: own directory, which is how each writes to the right place.
#:
#: This list started as the two artifacts #205 was written for and was WRONG to
#: stop there — that PR claimed "every remaining tracked generated artifact is
#: covered", and two more existed the whole time. Both were found by grepping
#: for files that declare themselves GENERATED, which is the check to repeat
#: before making that claim again.
ARTIFACTS = [
    (HERE / "chassis" / "trunk.stl", HERE / "chassis" / "trunk_build.py"),
    (HERE / "viewer" / "robot_viewer.html", HERE / "gen_viewer.py"),
    # The posture-aware hfe/haa envelope. Its header says "GENERATED by
    # hfe_envelope.py — do not edit", and nothing checked that it still was.
    (HERE / "chassis" / "rom_envelope_table.py", HERE / "chassis" / "hfe_envelope.py"),
    # The sim model. test_urdf_sync compares the MJCF against the CAD, which is
    # a different question from "does it match what build_mjcf.py produces now" —
    # a change to damping, limits or armature would pass that and not this.
    (PROJ / "sim" / "nova_mjx" / "nova.xml", PROJ / "sim" / "nova_mjx" / "build_mjcf.py"),
]

#: Generated files that are MANUALLY copied elsewhere. The generator writes one
#: location; a human carries it to the other. Nothing enforced that, and the
#: destination here is the copy the ROBOT uses — nova_ops reads the envelope for
#: the posture backstop, while the chassis copy only feeds CAD gates. A stale
#: copy on the ROS side is a safety table that disagrees with the geometry it
#: was derived from, in the direction nobody would notice.
COPIES = [
...
def check(artifact: pathlib.Path, script: pathlib.Path) -> bool:
    ...
    elif artifact.suffix in (".py", ".xml"):
        # Deterministic text generators — byte comparison is exact and the
        # strongest available answer. Both were measured byte-identical on
        # 2026-07-30; if one ever gains a timestamp, compare content instead of
        # loosening this to "close enough".
        ok = committed == fresh
        detail = ("byte-identical" if ok
                  else f"differs ({len(committed)} -> {len(fresh)} bytes)")
    else:
...
def check_copy(src: pathlib.Path, dst: pathlib.Path) -> bool:
    ok = src.read_bytes() == dst.read_bytes()
    print(f"   {'OK   ' if ok else 'DRIFT'} {str(dst.relative_to(PROJ)):46s} "
          f"{'matches' if ok else 'DIFFERS from'} {src.relative_to(PROJ)}")
    return ok
