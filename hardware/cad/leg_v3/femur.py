...
def build_femur():
    ...
    # ---- TTL daisy-chain pass-throughs 16x9 rect slots ----
    # Slot on -X body face (cable IN from coax/upstream)
    femur = femur.cut(
        cq.Workplane("YZ")
    ...
    # Slot on +X body face (cable OUT toward knee/tibia)
    femur = femur.cut(
        cq.Workplane("YZ")
    ...
    # Slot on yoke arms (inner face) so cable can exit to tibia
    for sign in (-1, 1):
        notch_y = sign * (S3_GAP_Y/2 + S3_ARM_T/2)
        femur = femur.cut(
            cq.Workplane("XZ")
            .workplane(offset=notch_y - sign * (S3_ARM_T/2 + 0.5))
            .center(S3_AXIS_X - S3_ARM_X/2 + WIRE_SLOT_W, S3_ARM_Z/2)
            .rect(WIRE_SLOT_W, WIRE_SLOT_H)
