...
| Disc screw pattern (BOTH discs) | 4× **M3** on Ø14 BCD ±45° + center — 🔴 *was "M2.5"; O3.0 nominal confirmed 2026-08-02. ✅ **TAPPED M3 — BENCH-CONFIRMED 2026-08-03, re-confirmed 08-07** (an M3 hand-threads in; #262 closed), so the tapped-vs-PA3.0-pilot question is settled. Note 4's ∅2.5="M2.5 clearance" reasoning stays retracted* | ✅ mesh (pattern), **bench** (thread) |
...
| **Mushroom CAP diameter** | **Ø40 (assumed)** | ⬜ **CALIPER NEEDED — and this one has a consumer.** The number appears NOWHERE in this table; it lives only in `chassis/control_pod.scad` comments, from this same vendor page. **DOWNGRADED 2026-08-10 (#35): this row no longer has a consumer.** `oled_mount` — whose whole position was a dodge around this cap, clearing the Ø40 circle by only **1.36 mm**, so that a real Ø42 was a −0.64 mm interference — is DELETED. The OLED now sits on `oled_tray`, flat on the rear shoulder deck starting at x−113 against a cap centred x−87: the rim would have to reach **Ø52** to touch it (Ø42.7 still leaves 4.65 mm). Still worth calipering for the panel-hole fit, but it no longer gates a mount. Two other rows from this same source are already ⚠️ REVIEW, and the SSD1331 outline from a vendor page was wrong by 4.9 × 5.1 mm — so treat Ø40 as unproven, not as a spec. 30 seconds with calipers settles it. |
...
| Hip grid: haa fore-aft × lateral spacing | **282.4 × 78.1** (half: 141.2 / 39.05) | ✅ same source; front/rear + left/right symmetric about (7.45, −14.2). ⚠️ **MIXED REFERENCE (2026-07-26): the 141.2 half comes from the stock HFE axis (Y −155.4 vs centre −14.2), the 39.05 half from the HAA horn (X −31.6). Legal for the stock robot** — the haa axis is fore-aft-parallel, so sliding along it is a kinematic no-op and putting the URDF's hip origin at the hfe station with `hip_to_upper_x = 0` is exact there. **NOT legal for v6**: `shoulder.scad` sets the haa station at ±141.2 (trunk end face ±63.5 + flange 77.7) and `coax.scad` puts the hfe axis 11.6 mm toward the trunk from it, i.e. **hfe at ±129.6** → real fore-aft pitch-axis spacing **259.2**, not 282.4. **RESOLVED 2026-07-27 (#165):** URDF `body_half_x` / MJX `MOUNT.x` keep 0.1412 (that IS the haa station) and the haa→hfe fore-aft term is now 11.6 mm toward the trunk, signed **per END** — so the pitch axes land at ±129.6 in both models. Guarded by `nova_locomotion/test/test_urdf_sync.py`, which reads this station from `check_fit.py` + `coax.scad` rather than a retyped constant. Row title keeps 282.4 because it names the **haa** grid; see the row below for the pitch grid. See `docs/cad-review-2026-07-26.md` §2 |
| v6 hfe (pitch) axis fore-aft station | **±129.6** (spacing 259.2) | ✅ derived 2026-07-26 from the built geometry: trunk end face ±63.5 (`trunk.stl` bounds) + `shoulder.scad` FLANGE_Y0 77.7 → haa ±141.2; `coax.scad` HFE_Y 11.6 lies along the haa axis, toward the trunk at BOTH ends once the rear crossmember is yawed 180° (`chassis/check_fit.py` `coax_to_trunk_bases()`) |
