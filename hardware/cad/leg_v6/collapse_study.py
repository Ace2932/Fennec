...
Motivated by the tibia_pad catch: the pad sits on the LATERAL blade face
(normal world +Y), so it never leads into the ground on a vertical collapse.
...
print(f"{'hfe':>4}{'kfe':>5} | {'coax':>7}{'femur':>7}{'tibia':>7}{'pad':>7}{'foot':>7} | lowest")
...
for hfe in (40,0,-50,50):
    for kfe in (109,55,0,-55,-109):
        ...
        print(f"{hfe:>4}{kfe:>5} | "+"".join(f"{lows[k]:7.0f}" for k in ('coax','femur','tibia','pad','foot'))
              +f" | {who} ({lows[who]:.0f})")
# tibia-specific: when tibia is low, is it the flat face or an edge? report the
# lowest tibia point's LOCAL coords + whether the pad is near it.
...
tw=tf(PTS['tibia'],W@Mtib); pw=tf(PTS['pad'],W@Mtib)
i=tw[:,2].argmin()
...
print(f"  pad lowest world z={pw[:,2].min():.0f}  (pad spans local x55..92 on the -Zlocal/+Yworld face)")
...
def leg_parts_haa(hfe,kfe,haa):
    ...
    return {
      ...
      'pad':  tp(PTS['pad'],  W@Mtib),
      'foot': tp(PTS['foot'], W@Mtib),
    }
print(f"{'hfe':>4}{'kfe':>5}{'haa':>5} | {'femur':>7}{'tibia':>7}{'pad':>7}{'foot':>7} | lowest / padgap")
for haa in (25,40):
    for hfe in (40,0):
        for kfe in (55,0,-55,-109):
            ...
            lows={k:v[:,2].min() for k,v in P.items()}
            who=min(lows,key=lows.get)
            padgap=lows['pad']-min(lows['femur'],lows['tibia'],lows['foot'])
            print(f"{hfe:>4}{kfe:>5}{haa:>5} | "+"".join(f"{lows[k]:7.0f}" for k in ('femur','tibia','pad','foot'))
                  +f" | {who} / pad {padgap:+.0f} above lowest")
