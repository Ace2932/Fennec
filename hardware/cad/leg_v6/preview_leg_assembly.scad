...
color([0.2,0.4,1,0.45])  // haa servo ghost
  rotate([0,-90,0]) rotate([90,0,0]) translate([-SPLINE_X,0,0]) sts3215_solid();

// femur hangs straight down from the hfe axis
translate([FEMUR_MID, HFE_Y, HFE_Z]) rotate([0,0,180]) rotate([0,90,0]) {
  femur_v6();
  color([1,0.2,0.2,0.45]) rotate([0,0,180]) translate([-SPLINE_X,0,0]) sts3215_solid();
  // tibia at the knee
  translate([FEMUR_LEN,0,0]) {
    tibia_v6();
    color([0.1,0.7,0.2,0.45]) rotate([0,0,180]) translate([-SPLINE_X,0,0]) sts3215_solid();
  }
}
