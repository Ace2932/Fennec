...
class ChoreoParams:
    ...
    crouch_z: float = 0.150  # low crouch, inside the kfe-109 floor
    lie_z: float = 0.140  # deepest commandable (kfe ~108°)
...
def pose_for(name: str, p: ChoreoParams) -> Pose:
    ...
    for leg in LEGS:
        ...
        if not within_limits(t_canon, p.leg, KNEE_FORWARD[leg]):
