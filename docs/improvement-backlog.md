...
| 35 | **Jetson cradle must respect the 2-part case (open end + bezel)** | CORRECTED 2026-07-08: `jetson_case_ref.stl` IS the real complete case (user-confirmed) — the 2 bodies = main shell + the slide-on end bezel, modeled ATTACHED at the open end (the y±7 red body is internal board/heatsink showing through the top vents, not a stray). ~1.2× oversize is just the mesh. The real issue: the case is a 2-part slide (shell + bezel that caps the OPEN END), so the cradle/placement must NOT force/jam that open end — today the port/open end sits 1.5 mm off the rear shoulder wall (jammed = no room for the bezel to attach/access in place). Resolve WITH #33/#34 (cradle redesign): confirm which end is open + give it clearance, or assemble the case fully (bezel on) off-robot then drop in. | **TODO (with #33/#34)** |
