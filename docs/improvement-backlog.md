...
| 38 | **Jetson cable routing exits the −y flank, not the rear end** | Ports are on the −y (robot-RIGHT) LONG face (confirmed #35), so the case cables + their right-angle plugs exit SIDEWAYS to −y — the current design routes them out the REAR end into the shoulder centre notch (`jetson_case_mount` header: "cables exit the −x port end"). Wrong wall. Re-plan: a −y-side cable path / cradle side-lip cutout on −y, and check the −y shoulder/deck for a drop to the C-box. | **TODO** |
