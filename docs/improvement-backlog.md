...
| 35 | **Jetson cradle must respect the 2-part case — orientation CONFIRMED** | `jetson_case_ref.stl` IS the real complete case (2 bodies = shell + internal board/heatsink; the removable **END BEZEL** slides on one short end). Orientation CONFIRMED w/ user 2026-07-08: long axis FORE-AFT (~110 along x, head +x); **PORTS/I-O on the −y face = robot's RIGHT flank** (my model was already correct — do NOT flip); bezel = a short-END cap; **case is assembled OFF-ROBOT (bezel on) then dropped into the cradle** (cradle just needs to clear + retain the finished box). Since the removable part is an END (not the top), capping the TOP corners is valid retention (grabs the fixed shell) — but see #33 (corner height) + #34 (drop-in). | **CONFIRMED — feeds #33/#34/#38** |
