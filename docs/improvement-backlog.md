...
| # | Item | Why | Status |
|---|---|---|---|
| CR-1 | **L2 mount pattern — physical confirm** | `l2_adapter.scad` L2_BCD=18 (36mm sq / Ø51@45°) is confirmed-correct per the L2 3D model (user); docs reconciled to it. Bench-measure the real L2 base to close. `head.scad` comment still says "22.5 square" — fix when next touching the head WIP. | **GATED: caliper the L2** |
...
| CR-3 | **head.scad HM insert bore ∩ snout-anchor bore** | Lower breakaway-insert bore (z89, Ø4) overlaps the #46 fox-mask snout-anchor bore (z86, Ø4) over x≈126.8–127.2 both sides → broken 360° wall for a structural insert / heat-set bleed-through. In the in-progress head-forward-neck work. | **TODO (in head WIP)** |
| CR-4 | **oled_mount SSD1331 holes in the display window** | 2 of 4 assumed mount holes land inside the window cutout (no material); the other 2 graze the edge. And the SSD1331 hole pattern itself is unverified (dimensions.md ⚠REVIEW). Can't mount as coded. | **TODO (verify real SSD1331 pattern first)** |
...
**VESTIGIAL remnants to clean up when next touching head.scad** (left as user WIP, not force-removed): the `#46 TPU FOX MASK` snout-line anchor bores in `head.scad` + the `fox_mask.stl` occluder hook in `head_fov_check.py` — both now dead (no fox mask). Remove at leisure.
