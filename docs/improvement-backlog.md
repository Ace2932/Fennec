...
| 38 | **Jetson cable routing exits the −y flank (STRAIGHT plugs)** | Ports on the −y (robot-RIGHT) LONG face (confirmed #35) → cables exit SIDEWAYS to −y. User has NO right-angle plugs → straight plugs stick out ~12-16 mm, past the ~4.8 mm channel → protrude the right flank = CRUSH risk in a side-fall. | **DONE 2026-07-08**: riser `CASE_SLOT` moved to the −Y flank (**x-30..30, y-51.5..-47**, inboard of the skirt → clean open drop into the bay, verified 0 obstruction). `jetson_case_mount` grows a **−Y CABLE COWL** (outer impact wall y-67 + 2 end walls tying to the −y uprights + a floor shelf) that SHIELDS the straight plugs (a −y side-fall hits the wall, not the plugs), open-top to plug in, floor guides cables to the CASE_SLOT → bay. Cantilevers ~12 mm past the riser −y edge (mid-body, verified clear of the legs). Dev RJ45 (~21) pokes above the open top — non-operational. ⚠ verify at wiring: bundle fit + the drop reaches the boards. |
