...
| CR-2 | **battery_pocket M3×12 → M3×16** | The 6 pocket→floor CSK screws engage the nut trap only ~0.3mm (raycast-confirmed; CSK head z5.9, tip z−6.1, nut top −5.8). Pack rides a strap so it won't drop, but the pocket-to-floor joint is inadequate. Go M3×16 (tip −10.1, full nut) or raise the nut trap ~2.5mm. | **TODO (design change)** |
...
| CR-5 | **neck_bracket front deck bolts in a 7mm corridor** | Drill-at-assembly pair (146,±19) has only 1.3–2.3mm margin between the deck window (y16) and horn-plate flange (y23) → hand-drill accuracy risk. Add a modeled pilot/locator or widen the corridor. | **TODO** |
| CR-6 | **femur/tibia vent windows vs bending margin** | Vents perforate BOTH pocket walls at the joint axis (max moment), removing ~43% of wall height over a 22mm span. Not a confirmed failure (torque mostly via horn discs; residual wall strips remain) but the SF15 slab calc predates the vent. Add a first-article static test point at the vent. | **TODO (first-article test)** |
| CR-7 | **Gate the newer chassis parts (was #39)** | `check_fit.py` never loads jetson_clamp_bar / jetson_cowl / l2_adapter / control_pod / oled_mount — that gap is why the +Y clamp-bar vs case grazing (~0.2mm, 4/13000 pts, uncertain vs mesh fidelity) went uncaught. Add them + settle the grazing. | **TODO** |
