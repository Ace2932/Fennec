...
1. **Gyro bias auto-zero on boot** — D456 IMU + L2 IMU (MPU-6050 cut per BOM v3.5; D456 + L2 are higher-quality). On node start, hold still for 5–10 s, average gyro readings, store as bias, subtract on output. L2 IMU bridge fix landed (`/unilidar/imu` @ 250 Hz via Ace2932/unilidar_sdk2 fork). Trivial as a ROS 2 service `~/calibrate_gyro_bias` plus auto-trigger on launch when `/cmd_vel` has been zero for N seconds. Accel bias is a separate, harder problem (needs known gravity orientation) — skip for v1; D456 has factory accel cal in its flash.
