...
| Servo-bus extension cables | leg runs > stock cable length | ❌ **Amazon order CANCELLED 2026-07-11** (seller couldn't fulfill — the **2nd** cable order to fall through, cf. the earlier "never received" Feetech order in the 🔴 list below). 🛒 **RE-SOURCE** the 6× 300mm + 6× 900mm (5264-3PIN, ~$19): AliExpress 5264-3P servo cables, a different Amazon seller, OR crimp exact lengths from bulk 3-core wire + 5264 contacts (⚠ the owned PEBA kit is listed only for **JST-XH** — confirm it has 5264 dies/contacts before relying on it). Crimping to the real measured lengths would ALSO close out #55 (the stock-length / loop-length uncertainty). Plan (unchanged when they arrive): 900s = chassis run + 3 between-leg links; 300s + the 12 stock in-box servo cables = within-leg links + VCC-pull donors (see wiring README dual-voltage recipe). Top-up trigger: count in-box cables at servo verify — if <12 or pull-mods eat spares, add the mixed 3×300+3×900 pack |
