...
| Thin solder 0.6-0.8 mm | 🔴 **verify — BLOCKS STAGE 1, and confirm the ALLOY too.** Diameter alone is not enough: every tip setpoint in `hardware/pcb-mods/BUILD_PLAN.md` §2a depends on the alloy. **Leaded Sn63Pb37 melts 183 °C, SAC305 217–220 °C** → a **+30 °C shift on every stage**, and the plane-tied pads get materially harder on lead-free (it could decide the preheat buy on its own). **Recommend leaded** — the logic board is HASL-lead, so its pads are already tin-lead; the power board is ENIG and takes either |
| Flux | ✅ owned (BEEYUIHF) |
