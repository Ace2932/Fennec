...
> The gate-harden (C_gs/D1/R_gs) ties to **BATT_NEG = drain**, not the literal source — still works: BATT_NEG ≈ GND ⚠ superseded by docs/power-chain-fmea.md#pc-01 (true only in normal polarity; reversed, D1 forward-biases and turns Q1 ON)
...
- ✅ **RESOLVED on v2 (verified 2026-07-30)** — ⚠️ **FOOTPRINT MISMATCH — board `nova_pcb_v6_power` U9/U10/U11.** The v2 footprints carry **4 pads only** (`+3V3`/`GND`/`SCL`/`SDA` at 2.54 mm): the colliding shunt pads are gone because the INA modules moved **off-board**, so rail current never touches the board and only I²C returns. That is why v2 DRC is 0 errors. The v1 analysis below is kept as the record of why the footprint changed. Original finding: `nova_v6:INA226_Module_Breakout` stacks pad 8 (Vbus) on pad 9 (IN−) at identical XY → "drilled holes co-located" DRC error, ×3. Real modules expose **VBUS as its own header pin** (not internally tied to IN−). Fix footprint AFTER picking the exact module: either give VBUS its own landing at the module's real pitch, or delete the board pad and wire VBUS→IN− externally. descr already flags "VERIFY pitch + header vs module before fab."
