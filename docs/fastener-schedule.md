...
| **HFE block → coax (retention)** ×2 (`coax_hfe_block.scad`, #226 option C) | **M3×16 SHCS** — ⚠️ **CORRECTED 2026-08-02, was M3×22 and that BOTTOMS OUT** | `M3_CLEAR` 3.4 through the tenon + Ø6.0 head c'bore on the block's outboard face | **M3×5.7 Ruthex insert**, bored −X from the mortise blind end — pocket **x 46.35 → 40.2** (RE-MEASURED off `coax_R.stl`) | **RE-MEASURED 2026-08-02: head seat x=57.0, insert pocket bottom x=40.2 → usable span 16.8 mm.** The previous row said the insert ended at x=37.6 and the span was 19.5 mm; that was true at #234 and **#235 moved `MORT_X0` 43.8 → 46.4**, taking the insert with it (`translate([MORT_X0 − HEATSET_L, …])`). The schedule was never re-measured. **M3×22 bottoms 5.2 mm early, M3×20 bottoms 3.2 mm early** — in a blind pocket, so the head never seats, there is no preload, and torquing jacks the block off the mortise or strips the insert. **M3×16** gives 5.35 mm of engagement in the 5.7 mm insert. Owned lengths are 10/14/20 → **M3×16 must be ordered**; M3×14 (3.35 mm ≈ 1.1×D) is a usable interim, M3×20 is NOT | ✅ retention only — the mortise/tenon carries the joint moment in bearing, so these see the cyclic 133 N pull-out split 2 ways = 66 N each vs ~175–245 N wet pull-out (SF 2.6–3.7). Driver access is +X open air — **verified 2026-08-02 with the femur SEATED**, clear for a 10 mm-diameter driver body |
