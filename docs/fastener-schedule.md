...
| **horn → HAA** ×4 (`shoulder_plate.scad`), **horn → KFE** ×4 (`knee_arm.scad`) | **M3×6** 🔴 *was M2.5×6* | `M3_CLEAR` **3.4** clear through `ARM_THK`=4.0, no head c'bore | into horn disc — **2.40 mm CALIPERED 2026-08-03**; BCD holes are **THROUGH** with **1.90 mm of air behind** (4.30 total). 🔴 *was "~3.05" — that is the raised central HUB, not the plate at R=7; see `dimensions.md`* | **grip 3.6mm MEASURED** (4.0 − 0.4mm `HORN_OD` locating-recess that spans the whole horn footprint) | ✅ M3×6 → **2.40 mm engagement = 100% of the 2.40 plate, tip exactly flush.** Cannot bottom — the hole is through with 1.90 mm behind. 💡 **The servo's own STOCK screw is M3×6**, so this position needs no purchase. 🔴 **THREAD CORRECTED TO M3 2026-08-02 (#263)** — the FEETECH STS3215 spec gives the disc threads as M3. Length math is unchanged (grip is geometry, not thread), but `M25_CLEAR` is **deleted** from `leg_v6_common.scad` |
| **HFE block → coax (retention)** ×2 (`coax_hfe_block.scad`, #226 option C) | **M3×16 SHCS** | `M3_CLEAR` 3.4 through the tenon + Ø6.0 head c'bore on the block's outboard face | **SLIM M3 heat-set: 4.0 mm OD × 6.0 long** — ✅ **SOURCED 2026-08-03: uxcell `B07R9SP532`** ("M3 x 6mm(L) x 4mm(OD)", M3×0.5, 50 pcs). ⚠️ **4.0 OD is NON-STANDARD for M3** (the industry table is hole 4.0 / OD 4.6 / wall 0.95) and **OD 4.0 is the M2.5 row**, so *check the thread on arrival, not just the OD* — a substituted part at this OD is most likely M2.5, which an M3×16 will not enter. In a **Ø3.5 × 6.5** bore at the mortise blind end (x 46.35 → 39.85). ⚠️ **NOT the 4.6 mm OD insert used everywhere else** | **RE-MEASURED 2026-08-02: head seat x=57.0, pocket bottom x=39.85 → 17.2 mm span; M3×16 engages 5.2 mm of the 6.0 insert (1.73×D) and stops 0.8 mm clear of the bottom. Pull-out π·4.0·6.0 = 75.4 mm² → SF 2.4–3.4, now gated.** Bolt axis moved z 11.5 → **11.7** to sit centred in the slot | ✅ **Why a different insert:** the insert must TRAVEL ~10 mm down the mortise to reach its bore, and that slot was **4.00 mm** against a 4.6 mm insert — undeliverable at any print accuracy, a valid seat with no path, the same failure that retired this joint's cap. Growing the slot to 5.0 for a 4.6 insert puts the roof at exactly 1.50 mm (`MIN_SECTION_MM`, no margin) and more forces `GROW_Z1` up, which **hits the shoulder at haa −40**. Pull-out goes as π·D·L, so 4.0×6.0 = **75 mm²** vs 4.6×5.7 = 82 mm² — **92 % of the strength for a 0.4 mm slot growth instead of 1.0**. `MORT_Z1` 13.5 → 13.9; roof 1.95 mm, floor 1.95 mm, both gated. Retention only — the tenon carries the moment in bearing (66 N/bolt cyclic vs ~160–225 N wet pull-out at this area, SF ≈ 2.4–3.4). `insert_path_checks()` now sweeps the insert's DIAMETER down its delivery path |
...
| **horn → HFE** ×4 (`coax.scad` inboard arm) | **M3×5** 🔴 *was M2.5×5* | same (`M3_CLEAR` **3.4**), but arm backs onto the HAA pocket cavity (LA-7: 3.2mm local budget, not the full 4.0mm `ARM_THK`) | into horn disc | **grip 2.8mm MEASURED** (3.2 local budget − 0.4mm recess; `ctr_deep=1.65` override on the center relief only) | ✅ M3×5 → **2.20 mm = 92% of the 2.40 plate.** 💡 Stock **M3×6 also works here**: 3.20 mm protrudes 0.80 mm past the plate into the 1.90 mm gap, it does NOT bottom. *(I earlier said a ×6 bottoms by 0.15 — computed against the wrong 3.05 figure AND a blind-hole assumption, both since disproved.)* |
...
| **stock retention screw** (driven side only — HAA/HFE/KFE horns, NOT wheels) ×1/servo | **STOCK** (Ø5.4 head × ~1.5mm proud, factory-installed with the horn) | printed clearance only: `HORN_CTR_D` Ø6.5 blind c'bore, `HORN_CTR_DEEP` 2.5mm generic. 🔴 **The "1.65mm at coax's inboard arm" figure in this row was WRONG — the built mesh measures 2.55mm** (probed on `coax_R.stl` 2026-08-03 along −X from the horn seat face x=16.05: solid starts x=13.50 across r 0..3, i.e. the full Ø6.5 pocket). The Ø20 disc locating recess measures 0.40–0.45 at r≥3.25, matching the documented 0.4. ⚠️ **The doc understated the margin, which is the safe direction but still wrong** — it had this joint looking like a 0.15mm squeak when the part has 0.55mm. **CLEARANCE CONFIRMED: hub stands 2.00mm proud** (CALIPERED 2026-08-03: disc 2.40, disc-bottom-to-hub-top 4.40) **against a 2.55 pocket → 0.55mm clear.** LA-7's first-article check is satisfied | n/a | n/a | ✅ NOT sourced — comes with the servo/horn kit |
