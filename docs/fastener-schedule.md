...
| **HFE block → coax (retention)** ×2 (`coax_hfe_block.scad`, #226 option C) | **M3×22 SHCS** | `M3_CLEAR` 3.4 through the tenon + Ø6.0 head c'bore on the block's outboard face | **M3×5.7 Ruthex insert** in the grown bridge, bored −X from the mortise blind end (x 43.8→37.6) | **MEASURED: head seats x=57.1, tenon grip 13.1mm, insert ends x=37.6 → 19.5mm span** → M3×22 (M3×20 is the minimum and leaves no seat margin) | ✅ retention only — the mortise/tenon carries the joint moment in bearing, so these see the cyclic 133 N pull-out split 2 ways = 66 N each vs ~175–245 N wet pull-out (SF 2.6–3.7). Driver access is +X open air |
