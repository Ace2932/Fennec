...
- **Trace current-capacity audit (2026-06-17) — PASS, 1 optional item.** Every high-current rail rides a pour/plane (audit-tool "UNDER" flags were false: thin traces are taps, not bulk path). V7V5_LEG → F.Cu pour + 1.5mm taps (~6A/tap); VBAT_PROTECTED → In2 plane; GND → In1 plane + B.Cu; BATT_NEG → F.Cu pour (1.3mm Q1 pin-escape short, pin-pitch limited); V12_HIP/ARM 3mm (~13A cap); V12_JET/L2/V5_AUX poured. **No vias on any power net** (no via-current risk). 
    - 🔴 **GUI in B1 reopen — set high-current pads to SOLID zone-connection (2026-06-17 audit).** All 3 power planes default to **thermal relief** (0.5mm spokes): VBAT_PROTECTED(In2), GND(In1/B.Cu), V7V5_LEG(F.Cu). BATT_NEG pour already SOLID ✓; Q1 fine (pad1=gate µA; 15A path Q1.2/Q1.3 already SOLID ✓); bucks VIN+GND already SOLID (the earlier 21-pad set). **Still on thermal relief in the main path — fix in GUI (multi-select → Pad Properties → Pad connection: Solid → re-pour; GUI pour trusted, headless re-pour has burned us):**
        - 🔴 **Buck VOUT pins U1.4/U2.4/U4.4/U5.4/U3.4** (rail output → pour). **#1 fix.** *Oversight: solidified VIN+GND but missed the outputs.* Probe confirms **pour-only, zero parallel tracks** → the **full rail** exits one pad's 4 thermal spokes (U1.4 leg ~22A peak/~8A cont, U2.4 hip ~11A) at ~90+ A/mm². Match the VIN pads.
        - 🟠 **SW1.2** (board power entry → In2 plane, ~13A continuous).
        - 🟡 J3–J7, J12–J14 injection XT30 (rail + GND, 6–8A) — nice-to-have.
        - ~~Bulk caps C1–C9 → SOLID~~ **SCRATCHED (over-stated 2026-06-17).** They're *electrolytics* (470µF/1000µF) absorbing inrush/servo transients at ms/low-freq — spoke inductance (~nH) is negligible vs the cap's own ESL/ESR (~10–20nH, tens of mΩ). SOLID matters for high-freq *ceramic* decoupling, not these. Leave on thermal relief.
        - Then me: re-pour-verify + DRC + regen gerbers + fab_gate.
    ...
    - **Audit closure (2026-06-17) — also verified clean:** GND return = **full-board In1 plane** (112×90mm, distributed low-Z, better than star — no neck/split). J20 mezzanine V5_AUX = **2 IDC pins** (~2–3A) + 5 GND pins → adequate for ~0.5–1A logic load (keeps the LED-status-only budget). IR drop <30mV all rails. No power vias. Q1/VBAT_PROTECTED-via cleared.
    - ✅ **Bulk-cap ripple/derating — checked 2026-06-17, low concern (→ B3 bench, not a blocker).** Roles: C8/C9 = 470µF/25V **Nichicon UPW1E471MPD** input bulk on VBAT_PROTECTED; C1–C5 = 1000µF/25V **FymuSing** (generic) output bulk at injection (J3–J7); C6 = 470µF/25V UPW on V12_L2.
        - **Voltage derating ✅ proven:** C8/C9 25V on 16.8V-max 4S = 1.49× (67% util, meets 80%); output caps 2–3.3×. (Confirms 35V→25V call.)
        - **C8/C9 input ripple — fine by topology:** Pololu modules carry on-module input ceramics (at VIN) → they shunt the HF switching RMS; electrolytics (high ESR/ESL @ HF) see only LF residual. UPW low-Z ~0.7A each → **2∥ ≈ 1.7–2A**, comfortable. Confirm split by scope at bring-up.
        - **C1–C5 output ripple — fine despite generic:** output position = inherently low ripple (LC + module output ceramic); these absorb **servo impact transients** (pulsed LF charge-delivery, not HF heating). FymuSing rating unspecified but low-stress → **IR-thermometer under sustained gait** to confirm.
        - Inrush ≠ ripple (one-time surge, caps handle it; SW1 contact wear is the real watch, already logged).
