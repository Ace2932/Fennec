...
class NovaJoystick(PipelineEnv):
    ...
    def step(self, state, action):
        ...
        # GATED on actual forward motion (fwd > 0.05 m/s): run 3 learned to MARCH
        # IN PLACE to farm this (traveled 0.00 m, reward 610) — feet lifting alone
        # paid the air reward. Now in-place stepping earns nothing; the base must
        # actually be moving the commanded way for air time to count.
        ...
        air_rew = jp.sum((air - 0.1) * first_contact) * cmd_moving * (fwd > 0.05)
