...
**All of it is regenerable — with one deliberate exception.** Everything else here
was generated by the CURRENT code; if a file looks stale or you are unsure it
matches HEAD, delete it and re-run the producer.

The exception is `policies/`, which holds **inputs**: PPO checkpoints trained in
Colab and downloaded from Drive. Nothing in this repo produces them, so deleting
one loses it. See `policies/README.md`.
...
| `policies/` | **nothing — INPUT.** Trained in Colab, downloaded from Drive | PPO checkpoints for `rollout.py --policy`, plus `eval_metrics.csv`. Gitignored except its README. Do not delete to "regenerate": there is no producer here. |
